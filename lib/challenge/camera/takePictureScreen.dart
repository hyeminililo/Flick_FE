...
class _TakePictureScreenState extends ConsumerState<TakePictureScreen> {
  late CameraController _cameraController;
  bool _isInitializing = false; // 초기화 상태를 관리하는 변수 추가
  ...
  Future<void> _switchCamera(CameraLensDirection direction) async {
    if (_isInitializing) return; // 초기화 중이면 반환
        ...
        children: [
          ...
          Positioned(
            ...
            child: CameraOptionsWidget(
              ...
              initializeCamera: _isInitializing
                  ? (direction) async {}
                  : _switchCamera, // 비활성화 조건 추가
            ),
          ),
          Positioned(
            bottom: screenHeight * 0.03,
          ...
          if (_isInitializing) // 로딩 인디케이터 표시
            const Center(
